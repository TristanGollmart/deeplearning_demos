...
def start_optimization_single_week(input_path, file_names, y, d, nHours):
    '''
    ...
    istart, iend = getIndexRange(input_path + r'Inputs\\' , file_names, y, d, nHours)

    capacities_HH = pd.read_csv(input_path + r'Inputs\\' + file_names['Capacities_HH'], header=None).values[::capacity_steps, 0]
    capacity_RF = pd.read_csv(input_path + r'Inputs\\' + file_names['CapacityPrices_RF'], header=None).values[0, 0]
    capacities_RF_excess = [capacity_RF * 10 ** i for i in [-2, -1, -0.5]]
    for capacity_HH in capacities_HH:
        for capacity_RF_Excess in capacities_RF_excess:
            tstart = datetime.now()

            pyo_model = get_model(input_path + r'Inputs\\', file_names, y, capacity_HH, capacity_RF_Excess)
            data = load_data(input_path + r'Inputs\\', file_names, pyo_model, istart, iend)
            ...
            print("number of minutes for a single optimization {}".format((datetime.now() - tstart).total_seconds() / 60))
...
def start_solver(model_instance):
    ...
    #opt.options['acceptable_iter'] = 10
    ...
    if (results.solver.status == SolverStatus.ok) and (results.solver.termination_condition == TerminationCondition.optimal):
        ...
        # something else is wrong: Write latest results to a new result object manually
        print(str(results.solver))
        # results_dict = {}
        # for var in model_instance.component_data_objects(pyo.Var):
        #     results_dict[var.name] = var.value
        # opt.options['max_iter'] = 1
        # opt.options['tol'] = 10e+10
        # new_results = opt.solve(model_instance, tee=False)
        # for var in model_instance.component_data_objects(pyo.Var):
        #     new_results.solution.variable[var.name] = results_dict[var.name]
        # return new_results
        opt.options['max_iter'] = 600
        opt.options['tol'] = 1e+4
        results = opt.solve(model_instance, tee=True)

    return results

def load_data(file_path, file_names, model, istart: int, iend: int):
    ...
    #tstart = d * nHours
    #tend = min((d+1) * nHours, len(pd.read_csv(file_path + file_names["T"])))
    t_index = [t for t in range(istart, iend)]
...
def get_model(input_path, file_names, y, capacity_HH, capacity_RF_excess):
    '''
    inputs:
    ...
    model.cap_RF_Excess = pyo.Param(domain=pyo.NonNegativeReals, initialize=capacity_RF_excess) # initiale guess: same as HH
...
def getIndexRange(file_path, file_names, y, d, nHours):
    # returns index range for time series: (y,1,1) + [d*nHours, (d+1)*nHours]
    # week should not start on monday to enable battery usage from sunday charging -> monday decharging
    tstart = d # datetime.strptime("{}.01.01".format(y), "%Y.%m.%d") + timedelta(hours=d * nHours)
    tend = tstart + timedelta(hours=nHours)
    if datetime.weekday(tstart) == 0: # monday
        tstart = tstart + timedelta(days=1)
        tend = tend + timedelta(days=1)
    istart = (tstart - datetime.strptime("{}.01.01".format(y), "%Y.%m.%d")).total_seconds() / 3600
    iend = (tend - datetime.strptime("{}.01.01".format(y), "%Y.%m.%d")).total_seconds() / 3600
    iend = min(iend, len(pd.read_csv(file_path + file_names["T"])))
